...
void display() 
{
  ...
  if (numused) {
	...
	GLint light0posn, light0color;
	GLint lightnumUsed;

	for (int i = 0; i < numused; i++) {
		int lightNum = (i * 4);
		transformvec(&lightposn[lightNum], &lightransf[lightNum]);
		//std::cout << "\lights = " << lightcolor[lightNum] << " " << lightcolor[lightNum + 1] << " " << lightcolor[lightNum + 2] << " " << lightcolor[lightNum + 3] << " " << "\n";
	}
  ...
  modelview = modelview*(tr*(transf*sc));
  // YOUR CODE FOR HW 2 HERE.  
